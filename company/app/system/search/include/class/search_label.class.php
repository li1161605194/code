  ...
  /**
	...
	public function get_search_opotion($type, $classnow, $page){
		...
		if($type == 'page'){//模块列表页面搜索
			...
			$paras = load::sys_class('label', 'new')->get('parameter')->get_para($class123['class1']['module'], $class123['class1']['id']);
			...
			foreach($paras as $key => $val){
				if($val['para_list']){
					...
					foreach($val['para_list'] as $valpl){
						$urlnow = $paraurl;
						$urlnow[$id] = $valpl;
						$p['name'] = $valpl;
						$p['url'] = $para_url."&para=".urlencode(load::sys_class('auth', 'new')->encode(json_encode($urlnow)));				
