var codeBase = {
    ...
    nodesListFor: function(data) {
        ...
        if (now_nodes.length > 0) {
            ...
            for (var b = 0; b < now_nodes.length; b++) {
                html += codeBase.nodesListFor(now_nodes[b]);
            }
            html += '</ol>';
        } else {
    ...
    menuParentInit: function() {
        common.post_shtml_json("sys/dic/dicAllToTree", {}, function(res) {
            if (res && res.data.length > 0) {
                var html = '';
                for (var a = 0; a < res.data.length; a++) {
                    html += codeBase.nodesListFor(res.data[a]);
                }
                $("#nestable").html('<ol class="dd-list">' + html + '</ol>');
                main.initHtmlLayui();
                codeBase.listTreeInit();
            } else {
