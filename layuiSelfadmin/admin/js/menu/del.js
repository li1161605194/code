var delMenu = {
    ...
    nodesListFor: function(data, level_num) {
        ...
        html += '<option value="' + data.id + '">' + /*level_num + "级菜单" +*/ delMenu.fengeMMM(level_num) + data.name + '</option>';
        if (now_nodes.length > 0) {
            level_num = parseInt(level_num) + 1;
            for (var b = 0; b < now_nodes.length; b++) {
                if (now_nodes[b].nodes.length > 0) {
                    html += delMenu.nodesListFor(now_nodes[b], level_num);
                } else {
                    html += '<option value="' + now_nodes[b].id + '">' + /*level_num + "级菜单" +*/ delMenu.fengeMMM(level_num) + now_nodes[b].name + '</option>';
                }
            }
        }
        return html;
        ...
        common.post_shtml_json("ser/user/menuAllToTree", {}, function(res) {
            if (res && res.rows.length > 0) {
                ...
                for (var a = 0; a < res.rows.length; a++) {
                    html += delMenu.nodesListFor(res.rows[a], 1);
                }
    ...
    vmInit: function() {
        ...
        form.on('select(parentId)', function(data) {
            ...
            if (data.value != "") {
                vm.toggle = true;
                delMenu.getMenuTreeData(data.value);
            } else if (data.value == "") {
                vm.toggle = false;
            }
        });
    ...
    init: function() {
        delMenu.menuParentInit();
        delMenu.vmInit();
        form.on('submit(menu-add)', function(data) {
            common.post_shtml_json("ser/user/delMenu", data.field, function(res) {
                if (res && res.resultCode == 200) {
                    delMenu.menuParentInit();
                    layer.msg("删除成功!");
                    vm.toggle = false;
                    tab.tabRefresh("menu/del", 1);
                }
            });
            return false;
        });
